...
// Configuración activa del probador (sincronizada desde el frontend)
let configProbador = {
    producto_id: null,
    altura: 170,
    peso: 70
...
const configurarProbador = (req, res) => {
    const { producto_id, altura, peso } = req.body;

    if (producto_id != null) configProbador.producto_id = Number(producto_id);
    if (altura != null) configProbador.altura = Number(altura);
    if (peso != null) configProbador.peso = Number(peso);

    console.log(`⚙️ [Probador Config] producto_id=${configProbador.producto_id} | H=${configProbador.altura} | W=${configProbador.peso}`);

    return res.status(200).json({ success: true, config: configProbador });
};

const obtenerConfigProbador = () => configProbador;
...
module.exports = { procesarVestidorIA, configurarProbador, obtenerConfigProbador };